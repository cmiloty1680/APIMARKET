...
function ProductionPage() {
  ...
  // Función para cambiar el título del formulario y preparar datos
  const updateTextTitleForm = (texto, rowData) => {
    setAction(texto)

    if (texto === "Actualizar") {
      // Si es actualizar, preparamos los datos para el formulario
      ...
      // Si es registrar, limpiamos los datos
      setProduction(null)
    }
  }

  // Función para abrir/cerrar el modal del formulario
  const openModalForm = (isOpen) => {
    setIsOpen(isOpen)
    // Si cerramos el modal, limpiamos los datos
    if (!isOpen) {
      setProduction(null)
    }
  }
  ...
  const actions = {
    ...
    update: (rowData) => {
      // Esta función ahora se maneja en updateTextTitleForm
    },
  }
  ...
  return (
    <div className="flex h-screen bg-gray-200">
      ...
      {/* Modal que se abrirá al hacer clic en "Agregar" o "Editar" */}
      <ModalDialog
        isOpen={isOpen}
