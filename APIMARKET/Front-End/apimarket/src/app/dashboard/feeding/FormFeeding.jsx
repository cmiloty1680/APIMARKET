...
function FormFeeding({ buttonForm, feeding }) {
    ...
    async function handlerSubmit(event) {
        ...
        if (!fecha || !cantidad || !tipoAlimentacion || !valor || !deshive) {
...
const setDataFeedingForUpdate = () => {
    setFecha(feeding.fec_Feeding ? new Date(feeding.fec_Feeding).toISOString().split("T")[0] : "");
                ...
                <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="space-y-1">
                        ...
                        <select className="w-full px-3 py-1.5 border rounded-md text-sm" required value={deshive || ""} onChange={(e) => setDesHive(e.target.value)}>
                            <option value="" >Seleccione</option>
                            {hives.map((hive) => (
                                <option key={hive.id_Hive} value={hive.id_Hive} disabled>{hive.des_Hive}</option>
                            ))}
                        </select>
                    </div>
                </div>
