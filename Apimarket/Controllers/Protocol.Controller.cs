...
namespace Apimarket.Controllers 
{
    ...
    public class ProtocolController : ControllerBase
    {
        ...
        public IActionResult AddP([FromBody] Protocol entity)
        {
            ...
            if (entity.Id_Protocol <= 0)
            {
                return BadRequest("El Id de Protocolo debe ser un valor positivo");
            }


            var existingImplement = _protocolServices.GetProtocol(entity.Id_Protocol);
            if (existingImplement != null)
            {
                return Conflict("ya existe una Protocolo con ese ID");
            }
            _protocolServices.Add(entity);
            return Ok("Protocolo creada con exito");
        }
        [Authorize]
